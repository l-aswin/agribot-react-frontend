...
export default function DeviceManager() {
  ...
  const [favoriteDeviceIds, setFavoriteDeviceIds] = useState(
    () => new Set(JSON.parse(localStorage.getItem('favoriteDeviceIds') ?? '[]'))
  );
  const [dashboardDeviceIds, setDashboardDeviceIds] = useState(
    () => new Set(JSON.parse(localStorage.getItem('dashboardDeviceIds') ?? '[]'))
  );

  // Delete modal
  const [deleteTarget, setDeleteTarget] = useState(null); // device object
  ...
  const filtered = devices.filter(d => {
    ...
    const matchFav  = filterFavs ? favoriteDeviceIds.has(d.id) : true;
    return matchName && matchDate && matchFav;
  });

  const activeDashCount = devices.filter(d => dashboardDeviceIds.has(d.id)).length;
  ...
  function handleNameChange(val) {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        setNameStatus('available'); // fallback: allow
      }
    }, 500);
  }
  ...
  const canCreate =
    ...
    CONFIG_FIELDS.every(f => form[f.key].trim() !== '');
  ...
  function toggleFavDevice(id) {
    setFavoriteDeviceIds(prev => {
      ...
      localStorage.setItem('favoriteDeviceIds', JSON.stringify([...next]));
      return next;
    });
  }
  ...
  function toggleDashDevice(id) {
    setDashboardDeviceIds(prev => {
      ...
      localStorage.setItem('dashboardDeviceIds', JSON.stringify([...next]));
      return next;
    });
  }
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50 font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-14 bg-white border-b border-slate-200 px-4 flex items-center gap-3 shrink-0">
          ...
          <h1 className="text-lg font-bold text-slate-800">Device Manager</h1>
        </header>
              ...
              <table className="w-full text-sm min-w-[600px]">
                ...
                <tbody className="divide-y divide-slate-100">
                  {loadingDevices ? (
                    ...
                    const isFav    = favoriteDeviceIds.has(d.id);
                    const isDash   = dashboardDeviceIds.has(d.id);
                    ...
                    <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-3 py-3 text-center">
                        <button
                          onClick={() => !dashFull && toggleDashDevice(d.id)}
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto p-6 space-y-4">
            ...
            {/* Device name with uniqueness check */}
            <div>
